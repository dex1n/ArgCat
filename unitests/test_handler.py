"""All UnitTests for ArgCat's handler"""
from argcat import ArgCat
from unitests.argcat_unittest import ArgCatUnitTest

# pylint: disable=unused-argument
class NormalHandlerProvider:
    """A normal handler provider."""

    @ArgCat.handler(parser_name='main')
    def main_handler(self, test: str=None) -> str:
        """Mock handler for the main parser.
        This signature matches the parser's argument definition."""
        return 'main'

    @ArgCat.handler(parser_name='init')
    def init_handler(self) -> str:
        """Mock handler for the subparser `init`."""
        return 'init'

    @ArgCat.handler(parser_name='info')
    def info_handler(self, detail: str=None) -> str:
        """Mock handler for the subparser `info`.
        This signature matches the parser's argument definition."""
        return 'info'

    @ArgCat.handler(parser_name='config')
    def config_handler(self, name: str=None, user_name: str=None) -> str:
        """Mock handler for the subparser `config`.
        This signature matches the parser's argument definition."""
        return 'config'

 # pylint: disable=unused-argument
class IncorrectHandlerProvider:
    """A handler provider provides handler with issues."""

    @ArgCat.handler(parser_name='git')
    def main_handler(self, test: str=None) -> str:
        """Mock handler for the subparser `git` which actually does not exist."""
        return 'main'

    @ArgCat.handler(parser_name='init')
    def init_handler(self) -> str:
        """Mock handler for the subparser `int` which is correct."""
        return 'init'

    @ArgCat.handler(parser_name='init')
    def xxxx_handler(self) -> str:
        """Mock handler for the subparser `int` which is redundant.
        Same parser name should not be added because there has already been one above. """
        return 'xxxx'
...
class DifferentKindsOfHandlerProvider:
    """A handler provider provides different kinds of handlers."""
    ...
    def normal_function_init_handler(self) -> str:
        """A normal function handler"""
        return 'normal_function_init'

class TestHandler(ArgCatUnitTest):
    """UnitText class for ArgCat's handler."""
    ...
    def test_incorrect_handler_provider(self) -> None:
        """Test incorrect handler providers."""
    ...
    def test_different_kinds_of_handler_provider(self) -> None:
        """Test all kinds of handler providers."""
    ...
    def test_add_normal_handlers(self) -> None:
        """Test adding normal handlers."""
        # Main handler
        def main_handler(test) -> str:
            return f'main_handler:{test}'
        self._argcat.set_parser_handler(parser_name='main', handler=main_handler)
        self.assertEqual(self._argcat._arg_parsers['main'].handler_func('parameter'),
                         'main_handler:parameter', "Parser handler for `main` is wrong!")
        ...
        self.assertEqual(self._argcat._arg_parsers['info'].handler_func('detail'),
                         'info_handler:detail', "Parser handler for `info` is wrong!")
        ...
        self.assertEqual(self._argcat._arg_parsers['config'].handler_func('Kevin', 'Cool-K'),
                         'config_handler:Kevin,Cool-K', "Parser handler for `config` is wrong!")

    def test_add_incorrect_handlers(self) -> None:
        """Test adding incorrect handlers."""
        ...
        self._argcat.set_parser_handler(parser_name='main',
                                        handler=main_handler_without_required_args)
        self.assertEqual(self._argcat._arg_parsers['main'].handler_func,
                         self._argcat._default_main_handler,
        ...
        self.assertEqual(self._argcat.set_parser_handler(parser_name='info',
                                                         handler=info_handler_without_args), False,
        ...
        self.assertEqual(
            self._argcat.set_parser_handler(parser_name='config',
                                            handler=config_handler_without_enough_args), False,
