"""All UnitTests for ArgCat's build()"""
...
class TestBuild(ArgCatUnitTest):
    """Test class for ArgCat's build()"""
    def setUp(self):
        self._argcat = ArgCat()

    # pylint: disable=too-many-statements
    def test_normal_build(self) -> None:
        """Test ArgCat's build() in a very normal scenario."""
        with self._argcat.build() as builder:
            ...
            #builder.add_argument('main', 'foo', '-o', '--fooa', action='store_false')
            ...
            builder.add_subparser('process',
                                  description='This is a sub parser for processing file or link.')

            # Add group for the sub parser
            builder.subparser('process').add_group(group_name='process_group_1',
                                                    description='This is the first group for \
                                                        process, which is not mutually exclusive',
                                                    is_mutually_exclusive=False)

            builder.subparser('process').add_group(group_name='process_group_2',
                                                    description='This is the second group for \
                                                    process, which IS mutually exclusive',
                                                    is_mutually_exclusive=True)

            # Add argument to sub parsers
            builder.subparser('process').add_argument('-f', '--file',
                                 nargs='?', dest='filename', type='str', group='process_group_1',
                                 required=True, help='The target file name.')
            builder.subparser('process').add_argument('-s', '--size', nargs='1', dest='filesize',
                                                      type='int', group='process_group_1',
                                                      required=False, default='1024')
        # pylint: disable=protected-access
        self.assertNotEqual(self._argcat._arg_parsers['main'], None,
                            "`main` parser should be created by default!")
        self.assertNotEqual(self._argcat._arg_parsers['process'], None,
                            "`process` parser should be created!")
        ...
        self.assertEqual(main_parser.dests, ['verbose', 'debug'],
                         "`main` parser's dests are incorrect!")
        self.assertNotEqual(main_parser.handler_func, None,
                            "`main` parser's handler should not be None, \
                                since it has a default one!")
        self.assertNotEqual(main_parser.parser, None,
                            "`main` parser's argument parser should be valid!")
        ...
        self.assertNotEqual(verbose_argument, None,
                            "`main` parser's `verbose` argument should not be None!")

        self.assertEqual(verbose_argument.option_strings, [],
                         "The `option_strings` of `verbose` argument is not correct!")
        self.assertEqual(verbose_argument.required, True,
                         "The `required` of `verbose` argument is wrong!")
        self.assertTrue(isinstance(verbose_argument, _StoreTrueAction),
                        "The `action` of `verbose` argument is wrong!")

        # Check the sub parser `process`
        process_parser = self._argcat._arg_parsers['process']

        self.assertEqual(process_parser.name, 'process',
                         "`process` parser's name is incorrect!")
        self.assertEqual(process_parser.dests, ['filename', 'filesize'],
                         "`process` parser's dests are incorrect!")
        self.assertEqual(process_parser.handler_func, None,
                         "`process` parser's handler should be None!")
        self.assertNotEqual(process_parser.parser, None,
                            "`process` parser's argument parser should be valid!")
        ...
        self.assertEqual(filename_arg.nargs, '?',
                         "The `nargs` of `filename` argument should be `?`!")
        self.assertTrue(filename_arg.type is str,
                        "The `type` of `filename` argument should be str!")
        self.assertEqual(filename_arg.option_strings, ['-f', '--file'],
                         "The `option_strings` of `filename` argument is not correct!")
        self.assertEqual(filename_arg.metavar, None,
                         "The `metavar` of `filename` argument is wrong!")
        self.assertEqual(filename_arg.help, "The target file name.",
                         "The `help` of `filename` argument is wrong!")
        self.assertEqual(filename_arg.required, True,
                         "The `required` of `filename` argument is wrong!")
        self.assertEqual(filename_arg.default, None,
                         "The `default` of `filename` argument should be None!")

        group_name = process_parser.additional_arguments_info[filename_arg.dest]['group']
        self.assertEqual(group_name, 'process_group_1',
                         "The `group` of `filename` argument is wrong!")
        ...
        self.assertEqual(filesize_arg.nargs, '1',
                         "The `nargs` of `filesize` argument should be `1`!")
        self.assertTrue(filesize_arg.type is int,
                        "The `type` of `filesize` argument should be int!")
        self.assertEqual(filesize_arg.option_strings, ['-s', '--size'],
                         "The `option_strings` of `filesize` argument is not correct!")
        self.assertEqual(filesize_arg.metavar, None,
                         "The `metavar` of `filesize` argument is wrong!")
        self.assertEqual(filesize_arg.default, '1024',
                         "The `default` of `filesize` is incorrect!")
        self.assertEqual(filesize_arg.required, False,
                         "The `required` of `filesize` argument is wrong!")
        ...
        self.assertEqual(len(process_parser.groups), 2,
                         "The number of groups for `process` parser is wrong!")
        ...
        def process_handler_with_incorrect_parameters(filename: str, the_type: int) -> str:
            return f"process_handler: {filename}, {the_type}"
        self._argcat.set_parser_handler(parser_name='process',
                                        handler=process_handler_with_incorrect_parameters)

        self.assertEqual(process_parser.handler_func, None,
                         "`process` parser's handler should be None after receiving \
                             an incorrect handler!")

        # Set a handler with correct paramters
        def process_handler_with_correct_parameters(debug: bool, filename: str,
                                                    filesize: int) -> str:
            return f"process_handler: {debug}, {filename}, {filesize}"
        self._argcat.set_parser_handler(parser_name='process',
                                        handler=process_handler_with_correct_parameters)

        self.assertEqual(process_parser.handler_func, process_handler_with_correct_parameters,
                         "`process` parser's handler should be valid after receiving \
                             a correct handler!")
